#!/bin/bash
# func: pull code and run hims container with 3 app
# date: 2018-01-03
# author: zhouwei
# email: xiaoamo361@163.com
# modify: 2018-03-19:
#         2018-03-20: copy from echeck use for hims
#         2018-04-09: add the git version check, to make the deploy must pull the new code already.

script_home=/home/tongxin/oem/hims
...
# pull code
old_version_num=$(su - tongxin -c "cd $script_home/txhims && git rev-parse HEAD")
su - tongxin -c "cd $script_home/txhims && git pull origin master"
if [ $? -ne 0 ]; then
...
new_version_num=$(su - tongxin -c "cd $script_home/txhims && git rev-parse HEAD")
