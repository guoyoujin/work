...
etcd_key=$1
# delte the key
curl --cacert $cacert --cert $cert --key $key $etcd_url/v2/keys/txcloud/$etcd_key?recursive=true -XDELETE
...
docker ps -a | grep $etcd_key | grep -v base | grep -v crond | while read line; do
	...
	curl --cacert $cacert --cert $cert --key $key $etcd_url/v2/keys/txcloud/$etcd_key/$app_key -XPUT -d value="$app_value"
done
